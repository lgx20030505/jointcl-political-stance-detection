...
from layers.gnn_layer import GraphAttentionLayer_weight
...
class GraphNN(nn.Module):
    ...
    def forward(self, node_feature, adj):
        for gnn_layer in self.gnn_layer_stack:
...
class BERT_SCL_Proto_Graph(nn.Module):
    ...
    def _bert_pool(self, concat_bert_indices, concat_segments_indices):
        outputs = self.bert(
            concat_bert_indices,
        ...
        if pooled_output is None:
            # fallback for models/configs without pooler output
    ...
    def forward(self, inputs):
        ...
        pooled_output = self._bert_pool(concat_bert_indices, concat_segments_indices)

        # Build adjacency matrix on correct device
        matrix = torch.zeros(
            [batch_size, centroids.shape[0] + 1, centroids.shape[0] + 1],
            device=self.device
        )
        matrix[:, -1:] = 1
        matrix[:, :, -1] = 1

        # Build graph node features on correct device
        feature = torch.zeros(
            [batch_size, centroids.shape[0] + 1, self.bert_dim],
            device=self.device
        )
        ...
        weight = weight[:, -1:]
        last_node_feature = torch.zeros([batch_size, self.bert_dim], device=self.device)
